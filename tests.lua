...
local function test(bits, wordbits)
  ...
  do --utils
    ...
    assert(bint(1):iszero() == false)
    assert(bint(2):iszero() == false)
    assert(bint.iszero(bint(0)) == true)
    ...
    assert(bint(2):isminusone() == false)
    assert(bint.isminusone(bint(-1)) == true)
    ...
    assert(bint.iseven(bint(2)) == true)
    assert(bint.iseven(bint(-2)) == true)
    ...
    assert(bint.isodd(bint(1)) == true)
    assert(bint.isodd(bint(-1)) == true)

    assert(bint.eq(bint(1), bint(1)) == true)
    assert(bint.eq(bint(1), bint(0)) == false)
    -- throws now
    -- assert(bint.eq(1.5, 1.5) == true)
    -- assert(bint.eq(1.5, 1) == false)
    -- assert(bint.eq(1.5, 2.5) == false)

    assert(bint(1):lt(bint(2)) == true)
    assert(bint(1):le(bint(2)) == true)
    -- 1e20 is too big for lua numbers, throws now
    -- assert(bint(1) <= bint(1e20) == true)
    -- need to be converted to number with zeros first
    assert(bint(1):lt(bint(string.format("%.0f", 1e20))) == true)
    ...
    assert(bint.eq(bint.fromstring('0xff'), bint(0xff)))
    assert(bint.eq(bint.fromstring('+0xff'), bint(0xff)))
    assert(bint.eq(bint.fromstring('-0xff'), bint(-0xff)))
    assert(bint.eq(bint.fromstring('0b10100'), bint(0x14)))
    assert(bint.eq(bint.fromstring('0b0'), bint(0)))
    assert(bint.eq(bint.fromstring('0b1'), bint(1)))
    assert(bint.eq(bint.fromstring('-0b1'), bint(-1)))
    assert(bint.eq(bint.fromstring('+0b1'), bint(1)))
    assert(bint.eq(bint.fromstring('0'), bint(0)))
    assert(bint.eq(bint.fromstring('-1'), bint(-1)))
    assert(bint.eq(bint.fromstring('1234'), bint(1234)))
    ...
    assert(bint.tole(bint(0)) == string.rep('\x00', bytes))
    assert(bint.tole(bint(0), true) == '\x00')
    assert(bint.tole(bint(1)) == '\x01' .. string.rep('\x00', bytes - 1))
    assert(bint.tole(bint(1), true) == '\x01')
    assert(bint.tole(bint(0x010203)) == '\x03\x02\x01' .. string.rep('\x00', bytes - 3))
    assert(bint.tole(bint(0x010203), true) == '\x03\x02\x01')
    assert(bint.tole(bint(-1)) == string.rep('\xff', bytes))
    assert(bint.tole(bint(-1), true) == string.rep('\xff', bytes))

    assert(bint.tobe(bint(0)) == string.rep('\x00', bytes))
    assert(bint.tobe(bint(0), true) == '\x00')
    assert(bint.tobe(bint(1)) == string.rep('\x00', bytes - 1) .. '\x01')
    assert(bint.tobe(bint(1), true) == '\x01')
    assert(bint.tobe(bint(0x010203)) == string.rep('\x00', bytes - 3) .. '\x01\x02\x03')
    assert(bint.tobe(bint(0x010203), true) == '\x01\x02\x03')
    assert(bint.tobe(bint(-1)) == string.rep('\xff', bytes))
    assert(bint.tobe(bint(-1), true) == string.rep('\xff', bytes))

    assert((bint.maxinteger() + bint(1)) == bint.mininteger())
    assert((bint.mininteger() - bint(1)) == bint.maxinteger())
    ...
    if bits > 96 then
      assert((bint(1) << bint(96)):gt(bint(string.format("%.0f", 1e28))))
      local neg_bint = (-(bint(1) << bint(96)))
      assert(neg_bint:lt(-bint(string.format("%.0f", 1e28))))
    end
  end
  ...
  do -- number conversion
    local function test_num2num(x)
      ...
      assert_eq(bint.tointeger(bint(x * 1.0)), x)
      assert_eq(bint.zero():_assign(bint(x)):tointeger(), x)
    end
    ...
    assert_eq(bint.tobase(bint(math.mininteger), 10), tostring(math.mininteger))
    assert_eq(bint.tobase(bint(math.maxinteger), 10), tostring(math.maxinteger))
    assert_eq(bint.tobase(bint(1), 10), '1')
    assert_eq(bint.tobase(bint(8), 7), '11')
    -- assert_eq(bint.tobase(bint(1), 37), nil)
    -- assert_eq(bint.tobase(bint(1.5), 10), nil)
    assert_eq(bint.tobase(bint(0xff), 16, true), 'ff')
    assert_eq(bint.tobase(bint(-0xff), 16, false), '-ff')
    if bits == 64 then
      assert_eq(bint.tobase(bint(-1), 16, true), 'ffffffffffffffff')
      assert_eq(bint.tobase(bint(-2), 16, true), 'fffffffffffffffe')
    elseif bits == 128 then
      assert_eq(bint.tobase(bint(-1), 16, true), ('f'):rep(32))
    end
  ...
  do -- add/sub/mul/band/bor/bxor/eq/lt/le
    local function test_add(x, y)
      ...
      assert_eq((bint(x):_add(bint(y))):tointeger(), x + y)
      -- assert_eq(bint(x) + (y + 0.5), x + (y + 0.5))
    end
    local function test_sub(x, y)
      assert_eq((bint(x) - bint(y)):tointeger(), x - y)
      assert_eq((bint(x):_sub(bint(y))):tointeger(), x - y)
      -- assert_eq(bint(x) - (y + 0.5), x - (y + 0.5))
    end
    ...
    local function test_lt(x, y)
      assert_eq(bint(x):lt(bint(y)), x < y)
    end
    local function test_ult(x, y)
      assert_eq(bint.ult(bint(x), bint(y)), math.ult(x, y))
    end
    local function test_le(x, y)
      assert_eq(bint(x):le(bint(y)), x <= y)
    end
    local function test_ge(x, y)
      assert_eq(bint(x):ge(bint(y)), x >= y)
    end
  ...
  do --shl/shr
    ...
    assert_eq(bint(1) << bint(math.mininteger), bint(0))
    assert_eq(bint(1) >> bint(math.mininteger), bint(0))
    assert_eq(bint.brol(bint(1), bint(math.mininteger)), bint(1))
    assert_eq(bint.bror(bint(1), bint(math.mininteger)), bint(1))
    assert_eq(bint.brol(bint(1), bint(-1)), bint.mininteger())
    assert_eq(bint.bror(bint(1), bint(1)), bint.mininteger())
    assert_eq(bint.brol(bint.mininteger(), bint(1)), bint(1))
    assert_eq(bint.bror(bint.mininteger(), bint(-1)), bint(1))
  end

  do -- pow
    local function test_ipow(x, y)
      assert_eq(bint.ipow(bint(x), bint(y)):tointeger(), math.floor(x ^ y))
      assert_eq(bint.ipow(bint(x), bint(y)):tointeger(), math.floor(x ^ y))
    end
    local function test_ops(x, y)
      test_ipow(x, y)
  ...
  do --tdivmod
    ...
    assert_eq(bint.tdiv(bint(7), bint(-3)):tointeger(), -2)
    assert_eq(bint.tdiv(bint(-7), bint(-3)):tointeger(), 2)
    assert_eq(bint.tdiv(bint(6), bint(3)):tointeger(), 2)
    assert_eq(bint.tdiv(bint(-6), bint(3)):tointeger(), -2)
    assert_eq(bint.tdiv(bint(6), bint(-3)):tointeger(), -2)
    assert_eq(bint.tdiv(bint(-6), bint(-3)):tointeger(), 2)

    assert_eq(bint.tmod(bint(7), bint(3)):tointeger(), 1)
    assert_eq(bint.tmod(bint(-7), bint(3)):tointeger(), -1)
    assert_eq(bint.tmod(bint(7), bint(-3)):tointeger(), 1)
    assert_eq(bint.tmod(bint(-7), bint(-3)):tointeger(), -1)
    assert_eq(bint.tmod(bint(6), bint(3)):tointeger(), 0)
    assert_eq(bint.tmod(bint(-6), bint(3)):tointeger(), 0)
    assert_eq(bint.tmod(bint(6), bint(-3)):tointeger(), 0)
    assert_eq(bint.tmod(bint(-6), bint(-3)):tointeger(), 0)

    -- assert_eqf(bint.tdiv(7.5, 2.2), 3.0)
    -- assert_eqf(bint.tmod(7.5, 2.2), 0.9)
    -- assert_eqf(bint.tdiv(-7.5, 2.2), -3.0)
    -- assert_eqf(bint.tmod(-7.5, 2.2), -0.9)
    -- assert_eqf(bint.tdiv(7.5, -2.2), -3.0)
    -- assert_eqf(bint.tmod(7.5, -2.2), 0.9)
  ...
  do -- upowmod
    assert_eq(bint.upowmod(bint(65), bint(17), bint(3233)):tointeger(), 2790)
    assert_eq(bint.upowmod(bint(2790), bint(413), bint(3233)):tointeger(), 65)
    assert_eq(bint.upowmod(bint(2790), bint(413), bint(1)):tointeger(), 0)
  end
end
